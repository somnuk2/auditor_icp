...
if ($request_data->action == "insert") {
    ...
    $member_id      = (int)$request_data->member_id;
    $query = "SELECT plc.plan_career_id, plc.start_date, car.career_name, cag.ca_group_name 
              FROM career             AS car
              INNER JOIN career_group AS cag ON car.career_group_id = cag.career_group_id
              INNER JOIN plan_career  AS plc ON plc.career_id = car.career_id
              INNER JOIN member       AS mem ON plc.member_id = mem.member_id
              WHERE plc.member_id = $member_id
              ORDER BY car.career_id asc 
             ;";
    $statement = $connect->prepare($query);
    $statement->execute();
    while ($row = $statement->fetch(PDO::FETCH_ASSOC)) {

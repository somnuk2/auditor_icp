...
//     $query = "SELECT sub.member_id, sub.Year,sub.M,sub.Month,sub.full_name, 
//                     car.career_name,sub.qualification_name,
//                     lev.level_name,
//                     tar.target_name, tar.target_value, 
//                     sub.minDay,per_1.perform_value AS min_perform_value,
//                     sub.maxDay,per_2.perform_value AS max_perform_value,
//                     sel_1.self_assessment_date AS date_start, sel_2.self_assessment_date AS date_final
//                 FROM ( SELECT  min(DATE_FORMAT(sel.self_assessment_date,'%e'))  AS minDay,
//                                 max(DATE_FORMAT(sel.self_assessment_date,'%e')) 	AS maxDay,
//                                 DATE_FORMAT(sel.self_assessment_date,'%m') 		AS M,
//                                 DATE_FORMAT(sel.self_assessment_date,'%M') 		AS Month,
//                                 DATE_FORMAT(sel.self_assessment_date,'%Y') 		AS Year,
//                                 mem.member_id, mem.full_name, qua.qualification_id, qua.qualification_name, sel.perform_id,
//                                 pla.career_id, qpc.target_id, qpc.level_id
...
//                             WHERE    DATE_FORMAT(sel.self_assessment_date,'%Y')  LIKE $year AND
//                                      mem.full_name             LIKE $full_name     AND 
//                                      qua.qualification_name    LIKE $qualification_name   

//                             GROUP BY mem.member_id, Year, M,  qpc.qualification_id
//                             ORDER BY mem.member_id, Year, M,  qpc.qualification_id ) AS sub

//                 INNER JOIN self_assessment AS sel_1 ON DATE_FORMAT(sel_1.self_assessment_date,'%e') = sub.minDay
//                 INNER JOIN self_assessment AS sel_2 ON DATE_FORMAT(sel_2.self_assessment_date,'%e') = sub.maxDay
...
//                 GROUP BY sub.member_id, sub.Year, sub.qualification_id
//                 ORDER BY sub.member_id, sub.Year, sub.qualification_id
//      ;";
//     $statement = $connect->prepare($query);
//     $statement->execute();
//     while ($row = $statement->fetch(PDO::FETCH_ASSOC)) {
...
//                 GROUP BY sub.member_id, sub.Year, sub.M, sub.qualification_id
//                 ORDER BY sub.member_id, sub.Year, sub.M, sub.qualification_id
//         ;";

...
if ($request_data->action == "insert") {
    ...
    $member_id = (int)$request_data->member_id;
    $query = "SELECT noti.notification_id,noti.member_id,noti.is_notification,
                     noti.notification_date,noti.message,
                     fre.frequency_id, fre.frequency_name,
                     mem.full_name, mem.status,ind.advisor_id

              FROM notification AS noti
              LEFT OUTER JOIN frequency      AS fre  ON noti.notification_type = fre.frequency_id
              LEFT OUTER JOIN member         AS mem  ON noti.member_id         = mem.member_id
              ...
              ORDER BY noti.notification_id ASC
              ;";
    $statement = $connect->prepare($query);
    $statement->execute();
    ...
    $member_id = (int)$request_data->member_id;
    $query = "SELECT mem.member_id,mem.full_name,mem.status,mem.created_by
              FROM member as mem
              LEFT OUTER JOIN individual     AS ind ON mem.member_id         = ind.member_id
